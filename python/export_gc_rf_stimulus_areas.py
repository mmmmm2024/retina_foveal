"""
Curcio由来の偏心度データ(Ecc_mm, X_mm)から、GC受容野を刺激平面(mm)へ変換、
midget/parasolの樹状突起式で受容野範囲と面積を計算してCSV出力し、円で可視化

入力:
- Curcio_RGC_Xvalues.xlsx（Ecc_mm, X_mm を含む）

出力:
- GC_Midget_StimulusAreas.csv
- GC_Parasol_StimulusAreas.csv
- 刺激平面上の受容野円のプロット（表示）

主な処理:
- mm→deg(Watson式)とdeg→刺激平面mm(視距離500mm)で座標変換
- 受容野の最小/最大角度、刺激平面上の中心X、面積(mm^2)と半径(mm)を計算
"""
